...
argsp = argsubparsers.add_parser("show-ref", help="List resfrences.")
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'Ruka <Ruka@example.com>'
        tag.kvlm[None] = b"A tag generated by Ruka, which won't let you customize the message!"
...
def ls_tree(repo, ref, recursive=None, prefix=""):
    ...
    for item in obj.items:
        ...
        if not (recursive and type=="tree"):
            ...
            ls_tree(repo, item.sha, recursive, ost.path.join(prefix, item.path))
...
def show_ref(repo, refs, with_hash=True, prefix=""):
    for k, v in refs.items():
        if type(v) == str:
            print("{0}{1}{2}".format(v + " " if with_hash else "", 
